...
import { setProjects, type Project } from "@/lib/projects";

export async function POST(req: Request) {
  if (!(await isAuthed())) return NextResponse.json({ ok: false }, { status: 401 });
  const body = (await req.json().catch(() => null)) as
    | { settings?: SiteSettings; products?: Product[]; projects?: Project[]; posts?: Post[] }
    | null;
  ...
  const okR = body.projects ? await setProjects(body.projects) : true;
  const okB = body.posts ? await setPosts(body.posts) : true;

  // refresh cached site content
  revalidatePath("/", "layout");

  return NextResponse.json({ ok: okS && okP && okR && okB });
}
