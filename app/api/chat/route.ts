...
export async function POST(req: Request) {
  ...
  } (or mirror the user's language). Help visitors understand the studio and pick the right engagement. Pricing is negotiated per project, not fixed tiers — it depends on scope, timeline and team needed. Never quote a number; offer a free scoping call instead. To start a project, point them to the contact form on the page or hello@plansio.studio. Never invent services, clients, prices or facts beyond these. Keep replies under ~90 words.\n\nOur products:\n${productList}`;
